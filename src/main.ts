...
export class Block {
  ...
  set(data: any, text: string | null = null):void {
    if (data.length === undefined) {
      const arrData = [data];
      const nd: BlockType[] = [];
  ...
  delete(option:number|string|null=null):BlockType|undefined {
  ...
  get(option:number|string|null=null): BlockType|undefined|BlockType[] {
    if (option !== null && typeof option === "number") {
      const index = option
      const exist = this.blocks.filter((_, i) => i === index)
  ...
  loop(callBack:any):void {
   ...
   /**@method merge two Blocks together */
  merge(cblock:Block):void {
